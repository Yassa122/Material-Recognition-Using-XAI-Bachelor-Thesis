...
import matplotlib.pyplot as plt  # For optional plot saving
...
class ChemBERTaForLogP(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, descriptors):
        ...
        return logp  # Removed .squeeze() to maintain batch dimension
...
def main():
    ...
    # 1. Load the Dataset
    data_path = "SMILES_Big_data_Set_100.csv"
    ...
    # 12. SHAP Explanations for Molecular Descriptors
    print("\nGenerating SHAP explanations for molecular descriptors...")

    # Select a subset of the validation data for SHAP explanations
    shap_sample_size = 100  # Adjust based on computational resources
    shap_background = train_df[descriptor_list].sample(n=10, random_state=42).values
    shap_explain = val_df[descriptor_list].sample(n=10, random_state=42).values

    # Define the prediction function for SHAP
    def shap_predict(descriptors_numpy):
        """
        Prediction function that takes only descriptors as input.
        To account for the SMILES input, a dummy SMILES string is used.
        """
        descriptors_tensor = torch.tensor(descriptors_numpy, dtype=torch.float).to(
            device
        )

        # Use a dummy SMILES string (e.g., "CCO" - Ethanol) repeated for the batch
        dummy_smiles = ["CCO"] * descriptors_numpy.shape[0]
        encoding = tokenizer(
            dummy_smiles,
            add_special_tokens=True,
            truncation=True,
        ...
        with torch.no_grad():
            predictions = model(input_ids, attention_mask, descriptors_tensor)

        # Ensure the output is a 1D numpy array
        return predictions.cpu().numpy().reshape(-1)

    # Initialize the SHAP KernelExplainer
    explainer = shap.KernelExplainer(shap_predict, shap_background)

    # Compute SHAP values for the explanation set
    shap_values = explainer.shap_values(shap_explain, nsamples=100)

    # Plot SHAP summary
    shap.summary_plot(shap_values, shap_explain, feature_names=descriptor_list)

    # Optionally, save the SHAP summary plot
    # shap.summary_plot(shap_values, shap_explain, feature_names=descriptor_list, show=False)
