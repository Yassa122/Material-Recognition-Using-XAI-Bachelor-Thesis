...
const ChatComponent = () => {
  ...
  // Example:
  const LIME_API_URL =
    "http://127.0.0.1:5000/api/explain_biological_activity_lime";
  const SHAP_API_URL = "http://127.0.0.1:5000/api/explain_biological_activity";

  // Fetch LIME and SHAP explanation data from the API
  useEffect(() => {
    const fetchLimeData = async () => {
      try {
        const response = await fetch(LIME_API_URL);
        const data = await response.json();

        // Suppose your /api/explain_biological_activity_lime returns something like:
        // {
        //   "status": "completed",
        //   "result": {
        //     "weights": [
        ...
        if (data.status === "completed") {
          ...
          setMessages((prev) => [
            ...
            {
              sender: "system",
              text: "LIME explanation loaded. See chart below!",
            },
          ]);
    ...
    const fetchShapData = async () => {
      try {
        const response = await fetch(SHAP_API_URL);
        const data = await response.json();

        // Suppose /api/explain_biological_activity returns:
        // {
        //   "status": "completed",
        //   "result": {
        //     "features": ["MolWt", "NumHDonors", ...],
        //     "shap_values": [[0.1, -0.2, ...], ...]
        //   }
        // }
        if (data.status === "completed") {
          ...
          // Just take first row if it's an array-of-arrays:
          const firstShapArray = shapResult.shap_values[0] || [];
          // Convert to chart data
          const shapChartData = features.map((feature, i) => ({
            label: feature,
            value: firstShapArray[i],
          }));
          ...
          setMessages((prev) => [
            ...
            {
              sender: "system",
              text: "SHAP explanation loaded. See chart below!",
            },
          ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:5000//api/chatgpt_bio_activity",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            limeData, // LIME data array
            shapData, // SHAP data array
          }),
        }
      );
